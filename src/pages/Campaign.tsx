...
const Campaign = () => {
  ...
  const fetchCampaignData = async () => {
    try {
      ...
      } = await supabase.from("assessments").select("*").eq("campaign_id", id).order("created_at", {
        ascending: false
      });
  ...
  const handleSendInvitation = async (assessment: Assessment) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke("send-assessment-invitation", {
        body: {
  ...
  const handleDeleteCandidate = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from("assessments").delete().eq("id", candidateId);
      if (error) throw error;
  ...
  return <div className="min-h-screen bg-background">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <Card className="p-6 bg-card border-border sticky top-6">
                ...
                <form onSubmit={handleAddCandidate} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="name" value={newCandidate.name} onChange={e => setNewCandidate({
                    ...newCandidate,
                    name: e.target.value
                  })} placeholder="Nome do candidato" className="bg-background border-border" />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="email">Email *</Label>
                    <Input id="email" type="email" value={newCandidate.email} onChange={e => setNewCandidate({
                    ...newCandidate,
                    email: e.target.value
                  })} placeholder="email@exemplo.com" required className="bg-background border-border" />
                  </div>

                  <Button type="submit" className="w-full bg-primary hover:bg-primary-hover" disabled={addingCandidate}>
                    {addingCandidate ? "Adicionando..." : <>
            ...
            <div className="lg:col-span-2">
              ...
              {assessments.length === 0 ? <Card className="p-8 text-center bg-card border-border">
                  ...
                  {assessments.map(assessment => <Card key={assessment.id} className="p-6 bg-card border-border hover:bg-card-hover transition-smooth">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {assessment.send_attempts > 0 && <Badge variant="outline" className="text-xs">
                                {assessment.send_attempts} {assessment.send_attempts === 1 ? "tentativa" : "tentativas"}
                              </Badge>}
                            {assessment.last_error_message && <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger>
                        ...
                        <div className="flex items-center gap-2">
                          {assessment.status === "completed" ? <Button variant="default" size="sm" className="bg-success hover:bg-success/90" onClick={() => navigate(`/results/${assessment.id}`)}>
                              <FileText className="w-4 h-4 mr-2" />
                              Ver Relatório
                            </Button> : <Button variant={assessment.status === "failed" ? "destructive" : "outline"} size="sm" className={assessment.status === "failed" ? "" : "border-primary text-primary hover:bg-primary hover:text-primary-foreground"} disabled={!canResend(assessment) || sendingInvitation === assessment.id} onClick={() => handleSendInvitation(assessment)}>
                              {assessment.status === "pending" ? <Mail className="w-4 h-4 mr-2" /> : <RefreshCw className="w-4 h-4 mr-2" />}
                              {getButtonText(assessment, sendingInvitation === assessment.id)}
                            </Button>}
                          <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => setCandidateToDelete(assessment)} disabled={deletingCandidate === assessment.id}>
      ...
      <AlertDialog open={!!candidateToDelete} onOpenChange={open => {
      if (!open && !deletingCandidate) {
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteCandidate} disabled={!!deletingCandidate} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
